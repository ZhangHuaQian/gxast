...
<body>
	<blockquote class="layui-elem-quote"
		...
		<div class="layui-input-inline">
			<input type="text" value="" placeholder="帮扶村名" id="name"
				autocomplete="off" class="layui-input">
		</div>
		<button type="reset" class="layui-btn" onclick="selectByObjecy()"><i class="layui-icon">&#xe615;</i>查找
	...
	<script>
		...
		function deleteUser(data, obj) {
			...
			//按钮
				...
				$.ajax({
					type : 'post',
					url : '${ctx }sys/village/delete?id=' + data.id,
					dataType : 'json',
		...
		function getUserData(pageNum) {
			...
			$.ajax({
				type : 'get',
				url : '${ctx }sys/village/selectList?pageNum=' + pageNum
						+ '&name=' + Name,
				dataType : 'json',
				success : function(result) {
					console.log(result);
		...
		function initTable(tableData) {
			layui.use('table', function() {
				...
				table.render({
					...
					, {
						field : 'pillarIndustry',
						title : '支柱产业',
						width : 200,
						align : 'center',
					}, {
						field : 'perCapitaIncome',
						title : '年人均收入',
						align : 'center',
						sort : true
					}, {
				...
				table.on('tool(testTable)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if (layEvent === 'update') { //编辑
						cen_showWin({
							...
							type : 2,
							title : '编辑帮扶村信息',
							url : '${ctx}sys/village/toupdate?id=' + data.id
						});
						...
						cen_showWin({
							...
							type : 2,
							title : '查看帮扶村信息',
							url : '${ctx}sys/village/village?id=' + data.id
						});
