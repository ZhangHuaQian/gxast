 ...
 <%--<%@ include file="/commViews/head.jsp"%>--%>
  ...
  <div class="layui-form-item" >
    ...
    <div class="layui-form-item">
      ...
      <div class="layui-input-inline">
        <input type="text" name="editor" readonly id="editor" placeholder="请输入编辑"  autocomplete="off" class="layui-input" >
      </div>
...
function closeWindow() {
	if($(window.parent.frames["家庭医药"]).length>0){
		window.parent.frames["家庭医药"].location.reload() ;// 刷新父窗口
		 setTimeout(function (){
...
function initContent(){
	$.ajax({
		type:'get',
		url: '${ctx }sys/jtyy/select?id=${jtyy.id}',
        //data:$('#main_form').serialize(),
        dataType: 'json',
        async:false, 
        success: function (result) {
            console.log(result);
            var jtyy=result.jtyy;
            $("input[name='title']").val(jtyy.title);
            $("input[name='orignfrom']").val(jtyy.orignfrom);
            $("input[name='editor']").val(jtyy.editor);
            $("input[name='author']").val(jtyy.author);
            $("input[name='description']").val(jtyy.description);
            NewsTypeId=jtyy.newTypeId;
            $("#demo3").html(htmlspecialchars_decode(jtyy.content));
            console.log(htmlspecialchars_decode(jtyy.content));
            console.log($("#demo3").html())

            $("#file").val(jtyy.file);
            if( jtyy.file !=null){
                var files = jtyy.file.split(",");
                var demoListView = $('#demoList');
                if(files[0]!=""){
...
function initNewsType(){
  ...
  function sendData(){
      ...
      if(layedit.getContent(layeditIndex).length > 0){
        $.ajax({
            type : 'post',
            url : '${ctx }sys/jtyy/update?id=${jtyy.id}',
            data : $('#main_form').serialize(),
            dataType : 'json',
            success : function(result) {
                if (result.status == 1) {
