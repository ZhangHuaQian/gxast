...
<script>
    ...
    function selectByObjecy() {
		...
		function getUserData(pageNum) {
			...
			$.ajax({
				type : 'get',
				url : '${ctx }sys/news_comment/selectList?pageNum=' + pageNum
			...
			layui.use('table', function() {
				...
				table.on('tool(testTable)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if (layEvent === 'update') { //编辑
					    console.log("status==="+data.commentstate)
                        //console.log(data);
                        if("未审核"==data.commentstate){
                            data.commentstate=1;
                        }else if("通过"==data.commentstate){
                            data.commentstate=1;
						...
						cen_showWin({
							...
							type : 2,
							title : '审核',
							url : '${ctx}sys/news_comment/update.jsp?id=' + data.id+'&commentstate='+data.commentstate,
						});
