...
<script>
    ...
    layui.use('form', function(){
        ...
        form.on('submit(formDemo)', function(data){
            ...
            $.ajax({
                type:'post',
                url:'${ctx }sys/upms/department/update',
    ...
    function initDep(){
        $.ajax({
            type:'get',
            url:'${ctx }sys/upms/department/selectById?did=${param.id}',
            data:$('#main_form').serialize(),
            success:function(result){
				var dep = result.department;
                $("input[name='id']").val(dep.id);
                $("input[name='name']").val(dep.name);
                $("input[name='orderNo']").val(dep.orderNo);
            },
            error:function (msg){
                console.log(msg);
                alert("系统异常");
