...
<body>
	...
	layui.use('laydate', function(){
		...
		function deleteUser(data, obj) {
			parent.layer.confirm('确定删除主题为' + data.title + '吗 ?', {
				btn : [ '确定', '取消' ]
			//按钮
			}, function() {
				//console.log(data);
				$.ajax({
					type : 'post',
					url : '${ctx }sys/nfkjw/delete?id=' + data.id,
					dataType : 'json',
    ...
    function insertUserShow() {
		...
		function getUserData(pageNum) {
			...
			$.ajax({
				type : 'get',
				url : '${ctx }sys/nfkjw/selectList?pageNum='+pageNum
				...
				table.on('tool(testTable)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if (layEvent === 'update') { //编辑
						cen_showWin({
							...
							type : 2,
							title : '编辑资讯',
							url : '${ctx}sys/nfkjw/toupdate?id=' + data.id
						});
                        ...
                        cen_showWin({
                            ...
                            type : 2,
                            title : '查看资讯',
                            url : '${ctx}sys/nfkjw/news?id=' + data.id
                        });
