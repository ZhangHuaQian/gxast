 ...
 <%--<%@ include file="/commViews/head.jsp"%>--%>
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-form-item">
      ...
      <div class="layui-input-block">
        <input type="checkbox" name="recommend" lay-skin="switch" lay-filter="recommend" lay-text="是|否">
        <span style="color: #adadad">(南方科技报-主页#6)</span>
      </div>
    </div>
...
function closeWindow() {
	if($(window.parent.frames["南方科技网"]).length>0){
		window.parent.frames["南方科技网"].location.reload() ;// 刷新父窗口
		 setTimeout(function (){
...
function initContent(){
	$.ajax({
		type:'get',
		url: '${ctx }sys/nfkjw/select?id=${nfkjw.id}',
        dataType: 'json',
        async:false, 
        success: function (result) {
            console.log(result);
            var nfkjw=result.nfkjw;
            $("input[name='title']").val(nfkjw.title);
            $("input[name='orignfrom']").val(nfkjw.orignfrom);
            $("input[name='editor']").val(nfkjw.editor);
            $("input[name='author']").val(nfkjw.author);
            $("input[name='description']").val(nfkjw.description);
            NewsTypeId=nfkjw.newTypeId;
            $("#demo3").html(htmlspecialchars_decode(nfkjw.content));

            if(nfkjw.recommend){
                $(":radio[name='recommend'][value='"+nfkjw.recommend+"']").attr("checked","true");
                $("input[name='recommendImg']").val(nfkjw.recommendImg);
                $('#preview').css('display','').attr('src','${ctxUphoto}'+nfkjw.recommendImg)
                $("#recommend_img").css("display","");
            }

            $("#file").val(nfkjw.file);
            if(nfkjw.file!=null){
                var files = nfkjw.file.split(",");
                var demoListView = $('#demoList');
                if(files[0]!=""){
...
function initNewsType(){
	$.ajax({
		type: 'get',
        url: '${ctx }sys/dictionary/selectList?type=type_nfkjw',
  ...
  function sendData(){
      ...
      if(layedit.getContent(layeditIndex).length > 0){
        $.ajax({
            type : 'post',
            url : '${ctx }sys/nfkjw/update?id=${nfkjw.id}',
            data : $('#main_form').serialize(),
            dataType : 'json',
            success : function(result) {
                if (result.status == 1) {
