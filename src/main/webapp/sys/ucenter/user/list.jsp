...
<%--<form class="layui-form layui-form-pane" onsubmit="return false" id="main_form" style="">--%>
    <blockquote class="layui-elem-quote" style="padding:10px;margin-bottom:0px;">
        ...
        <button class="layui-btn layui-btn-normal" onclick="insertUserShow()"><i class="layui-icon">&#xe608;</i>添加用户
        </button>
...
<script type="text/html" id="barDemo">
    ...
    <shiro:hasRole name="超级管理员"><button class="layui-btn layui-btn-xs layui-btn-warm" lay-event="changepsw"><i class="layui-icon">&#xe642;</i>密码重置
    </button></shiro:hasRole>
...
<script>
    ...
    function deleteUser(data,obj){
        parent.layer.confirm('确定删除' + data.loginname + '吗 ?', {
            btn: ['确定', '取消'] //按钮
        }, function () {
            $.ajax({
                type: 'post',
                //url: '${ctx }sys/ucenter/update?id='+data.id+'&isDel=1',
                url: '${ctx }sys/ucenter/delete?id='+data.id,
                dataType: 'json',
    ...
    //重置密码
    function changePSW(data,obj){
        parent.layer.confirm('确定需要重置' + data.name + '用户密码吗 ?', {
            btn: ['确定', '取消'] //按钮
        }, function () {
            //console.log(data);
            $.ajax({
                type: 'post',
                url: '${ctx }sys/ucenter/changepsw?id='+data.id,
                dataType: 'json',
                success: function (result) {
                    //console.log(result);
                    if(result.status==1){
                        parent.layer.msg('操作成功，默认密码与账号一致', {icon: 1});
                    }else{parent.layer.msg('操作失败', {icon: 5});}
    ...
    function getUserData(pageNum) {
        ...
        <%--console.log('${ctx }sys/ucenter/selectList?pageNum='+pageNum+'&loginname='+loginName+'&name='+userName);--%>
        $.ajax({
            type: 'get',
            url: '${ctx }sys/ucenter/selectList?pageNum='+pageNum+'&loginname='+loginName+'&name='+userName,
            dataType: 'json',
            success: function (result) {
                console.log(result);
                if(result.count > 0){
        ...
        layui.use('table', function () {
            ...
            table.render({
                ...
                , cols: [[ //表头
                    ...
                    , {field: 'loginname', title: '账号', align: 'center', width: 200}
                    , {field: 'phonenum', title: '手机', align: 'center', width: 200}
            ...
            table.on('tool(testTable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'update') {   //编辑
                    cen_showWin({
                        ...
                        type: 2,
                        title: '编辑用户',
                        url: '${ctx}sys/ucenter/user/update.jsp?id='+data.id
                    });
                    ...
                    cen_showWin({
                        ...
                        type: 2,
                        title: '查看用户',
                        url: '${ctx}sys/ucenter/user/select.jsp?id='+data.id
                    });
                } else if (layEvent === 'delete') {   //删除changepsw
                    deleteUser(data,obj);
                }else if (layEvent === 'changepsw') {   //重置密码
                    changePSW(data,obj);
                }
            });
