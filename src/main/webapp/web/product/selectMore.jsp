...
<html>
	...
	<head>
		<meta charset="utf-8">
		<title id="title">查看更多商品-广西科普传播中心</title>
			...
			<div class="bg" style="margin-top: 10px">
				<div style="width: 100%;min-width: 1162px;background-color: white; margin: 10px 0px;height: 40px; border: 1px solid #E8E8E8">
					<div id="app" style="margin: 13px;">
						...
						<c:if test="${param.type == 22}">
						<span><a href="${ctx }/web/product/home">小博士报<i class="layui-icon" style="margin-left: 5px;color: #c0c4cc">&#xe602;</i></a></span>
						</c:if>
						<c:if test="${param.type == 59}">
						<span><a href="${ctx }/web/product/home">家庭医药<i class="layui-icon" style="margin-left: 5px;color: #c0c4cc">&#xe602;</i></a></span>
						</c:if>
						<c:if test="${param.type == 60}">
						<span><a href="${ctx }/web/product/home">南方科技报<i class="layui-icon" style="margin-left: 5px;color: #c0c4cc">&#xe602;</i></a></span>
						</c:if>
						<span>查看更多</span>
	...
	<script>
		...
		$(function () {
			var type = '${param.type}'
			if(type ==22){
				$("#keyWord").attr("placeholder","搜索小博士报")
            }else if(type ==59){
                $("#keyWord").attr("placeholder","搜索家庭医药")
            }else if(type ==60){
                $("#keyWord").attr("placeholder","搜索南方科技报")
            }
            //搜索
        ...
        function initData(pageNum,key) {
            // if(key.length > 0){
                $.ajax({
                    type:'get',
                    url:'${ctx }/web/product/productSearch?pageNum='+pageNum+"&key="+key+"&type=${param.type}",
                    dataType:'json',
