        ...
        <div id="newslist_div" style="width:77%;float: left;background-color: #ffffff">
            <div id="newslist_area" style="margin-left: 10px;">
                <div id="newslist_header" style="color: GRAY;">
                    ...
                    </i>你当前位置 > <a href="${ctx }">首页</a> > <a >家庭医药</a> <a id="zhuti" ></a> > ${news.title}</span>
                    </div>
                </div>
...
<script>
    ...
    function detailid_find(){
        $(".find").click(function () {
            window.location.href='${ctx}web/news_jtyy/selectList?pageNum='+1+'&newTypeId='+$(this).attr("detailid");
        })
    ...
    layui.use([ 'form' ], function() {
        ...
        form.on('submit(formDemo)', function(data) {
            $.ajax({
                type : "POST",
                url : '${ctx}web/news_nfkjw/commitcomment.do?topic_type=jtyy',
                data:{id:${news.id},
                    content:$("#comment").val()
                },
                dataType:'json',
    ...
    function comment(){
        ...
        $.ajax({
            type : "POST",
            url : "${ctx}web/news_jtyy/comment.do?id=${news.id}&topic_type=jtyy",
            error : function(data) {
                layer.msg('网络异常');
            },
    ...
    function comment_two() {
        ...
        $.ajax({
            type : "POST",
            url : "${ctx}web/news_jtyy/comment_two.do?id=${news.id}&topic_type=jtyy",
            error : function(data) {
                layer.msg('网络异常');
            },
            success : function(data) {
...
<script>
    $(function () {
        ...
        $("#like").click(function () {
            $.ajax({
                type : "POST",
                url : "${ctx}web/news_jtyy/insert?id=${news.id}&news_type=jtyy",
                error : function(data) {
                    alert("网络异常 ！");
                },
                ...
                success : function(data) {
                    ...
                    if(data==2){
                        ...
                        $("#span").html(data.news_jtyy.praiseView);
                    }
                }
            });
    ...
    function select() {
        ...
        $.ajax({
            type : "POST",
            url : "${ctx}web/praise/select?id=${news.id}&news_type=jtyy",
            error : function(data) {
            },
            success : function(data) {
                console.log(data);
