        ...
        <div id="newslist_div" style="width:77%;float: left;background-color: #ffffff">
            <div id="newslist_area" style="margin-left: 10px;">
                <div id="newslist_header" style="color: GRAY;">
                    ...
                    </i>你当前位置 > <a href="${ctx }">首页</a> > 家庭医药 > <a id="zhuti"></a></span>
                ...
                <div id="newslist" style="margin-top: 10px;">
                    <div id="newslists" style="margin-right: 10px;">
                        <c:forEach items="${list}" var="list">
                            <ul><div class="news_list"><a target="_blank" href="${ctx}web/news_jtyy/details/${list.id}.html"><span style="">${list.title}</span></a><span class="news_list_time"><fmt:formatDate value="${list.createTime}" pattern="yyyy-MM-dd"/></span></div></ul>
                        </c:forEach>
                    </div>
                ...
                <ul>
                    <div style="background-color:#2377cd;color:#FFFFFF;border-bottom: 2px solid #f2f5f9;padding-top: 8px;padding-bottom: 8px;">
                        <div style="margin-left: 10px;">
                            ...
                            <span style="margin-left: 2px;">家庭医药</span>
                        </div>
...
<script>
    ...
    function search(){
        if("${param.newTypeId}"==""||"${param.newTypeId}"==null||"${param.newTypeId}"==undefined){
            window.location.href="${ctx}web/news_jtyy/selectList?pageNum="+1+"&title="+$("#title").val();
        }else{
            window.location.href="${ctx}web/news_jtyy/selectList?pageNum="+1+"&title="+$("#title").val()+"&newTypeId=${param.newTypeId}";
        }
    ...
    function dictionary(){
        ...
        $.ajax({
            ...
            data: {type: 'type_jtyy'},    //视情况将base64的前面字符串data:image/png;base64,删除
            dataType: "json",
            error: function(msg) {
                console.log(msg)
                alert("请检查网络后重试");
    ...
    function com_even_bind(){
        $(".commentsBtn").click(function () {
            window.location.href='${ctx}web/news_jtyy/selectList?pageNum='+1+'&newTypeId='+$(this).attr("comid");
        })
        ...
        layui.use([ 'laypage', 'layer' ], function() {
            ...
            laypage.render({
                ...
                jump : function(obj, first) {
                    if (!first) {//首次不执行
                        window.location.href= '${ctx }web/news_jtyy/selectList.html?pageNum='+ obj.curr+'&newTypeId=${param.newTypeId}';
