	...
	<div class="bg" style="margin-top: 10px;margin-bottom: 100px ">          
		<div                            
			...
			<div id="app" style="margin: 13px;">                
				...
				<el-breadcrumb-item> <i class="layui-icon">&#xe68e;
						...
						href="${ctx }/web/product/home">商城首页<i class="layui-icon" style="margin-left: 5px;color: #c0c4cc">&#xe602;</i></a></el-breadcrumb-item> <el-breadcrumb-item>订单支付</el-breadcrumb-item>
				</el-breadcrumb>
			</div>
		</div>
		...
		<div style="border: 1px solid #e8e8e8; padding: 10px;">
			<div style="display: inline-block">
				<img src="${ctxRes}images/wx-pay.jpg" style="width: 200px">
			</div>
			<div style="display: inline-block;margin-left: 30px;">
				<div style="margin-top: 20px;">
					<h2 id="name" style="font-weight: 900;"></h2>
				</div>
				<div style="padding-top: 30px;">
					<span>支付 </span>
					<span id="price" style="color: #ff8208;font-weight: 900;"> </span>
					<span> 元</span>
				</div>
				<p></p>
			</div>
			<div style="padding: 30px 10px 10px 10px;">
				<input class="pay-btn" type="button" value="立即支付" onclick="showQRCode()">
			</div>
		</div>
...
<script>                                 
	...
	$(function () {
        $.ajax({
            type: 'post',
            url: '${ctx }web/order/getOrderInfo',
            dataType: 'json',
            data: {
                way : '${param.way}',
                orderId : orderId,
	...
	function checkOrder() {
	    if (times > 0){
            $.ajax({
                ...
                success: function (result) {
                    if(result.status==1){
                        window.location.href = "${ctx }web/order/success.jsp?id=${param.orderId}"
                    }else if(result.status==5){
                        window.location.href = "${ctx }web/order/fail.jsp?id=${param.orderId}"
                    }
                },
                error: function (msg) {
                }
    ...
    function showQRCode() {
        ...
        qrCode = layer.open({
            ...
            btnAlign: 'c',  // 按钮居中对齐
            // content: 'http://wxdev.sx-tech.net/gxast/wx/wxPay?orderId=f83ad97f90cd11eab2a240f2e9100bb4',
            content: 'http://wxdev.sx-tech.net/gxast/wx/wxPay?orderId=${param.orderId}&orderIdList=${param.orderIdList}&way=${param.way}',
        });
    }
