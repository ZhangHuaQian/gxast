        ...
        <div id="newslist_div" style="width:77%;float: left;background-color: #ffffff">
            <div id="newslist_area" style="margin-left: 10px;">
                <div id="newslist_header" style="color: GRAY;">
                    ...
                    </i>你当前位置 > <a href="${ctx }">首页</a> > <a href="${ctx }web/news/selectList">新闻中心</a> <a id="zhuti"></a> </span>
...
<script>
    ...
    function searchTitle(){
        ...
        if("${param.newTypeId}"==""||"${param.newTypeId}"==null||"${param.newTypeId}"==undefined){
            window.location.href="${ctx}web/news/selectList?pageNum="+1+"&title="+$("#title").val();
        }else{
            window.location.href="${ctx}web/news/selectList?pageNum="+1+"&title="+$("#title").val()+"&newTypeId=${param.newTypeId}";
        }
            ...
            laypage.render({
                ...
                jump : function(obj, first) {
                    if (!first) {//首次不执行
                        ...
                        window.location.href= '${ctx }web/news/selectList.html?pageNum='+ obj.curr+'&newTypeId=${param.newTypeId}'+"&pageSize="+obj.limit;
