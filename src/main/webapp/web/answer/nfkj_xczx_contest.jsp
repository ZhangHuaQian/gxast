...
<style>
    body{
        margin: 0px;
        background-image: url('${ctx}resource/images/answer/xczx_content_bg.png');
        top: 0;
        left: 0;
        z-index:-10;
        zoom: 1;
...
<script>
    ...
    $(function () {
        ...
        $.ajax({
            type: "get",
            url: "${ctx}appsite/nfkj/xczxContestRecord/selectByUid?openid="+'${param.openid}',
            dataType: "json",
            success: function(result) {
                ...
                if (result.status == "0"){
                    ...
                    if (result.code){
                        window.location.href = 'nfkj_xczx_end.jsp?code='+result.code+'&msg='+result.msg
                    } else {
                        if (!result.data) {
                            $('#options').remove()
                            $.toast.prototype.defaults.duration=3000
    ...
    function choose(e) {
        ...
        $.confirm("您确定要提交此选项吗?", "确认选项?", function() {
            ...
            $.ajax({
                type: "post",
                url: "${ctx}appsite/nfkj/xczxContestRecord/insert",
