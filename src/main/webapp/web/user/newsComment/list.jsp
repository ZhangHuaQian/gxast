            ...
            <div class="fly-panel fly-panel-user" pad20="">
                ...
                <div class="layui-tab layui-tab-brief" lay-filter="user">
                    <ul class="layui-tab-title" id="LAY_mine">
                        <li data-type="mine-jie" lay-id="index" class="layui-this">资讯 - 我的评论</li>
...
<script>
    ...
    function selectmytopic(pageNum) {
        $.ajax({
            type: 'get',
            url: '${ctx }myNewsCommentList?&pageNum='+pageNum,
            ...
            success: function (result) {
                ...
                if(result.list.length>0){
                    for (var i = 0; i < result.list.length; i++) {
                        var colorClass;
                        if(list[i].commentstate==0){
                            list[i].commentstate="未审核";
                            colorClass = "layui-bg-orange";
                        }else if(list[i].commentstate==1){
                            list[i].commentstate="审核通过"
                            colorClass = "layui-bg-green";
                        }else if(list[i].commentstate==2){
                            list[i].commentstate="审核未通过"
                            colorClass = "layui-bg-red";
                        }

                        var href = "";
                        if(list[i].topicType == "xwzx"){ //新闻中心
                            href = '${ctx}web/news/details/'+list[i].newsId+'.html'
                        }else if(list[i].topicType == "djgz"){ //党建工作
                            href = '${ctx}web/news_axhd/details/'+list[i].newsId+'.html'
                        }else if(list[i].topicType == "zzgk"){ //政务公开
                            href = '${ctx}web/news_zzgk/details/'+list[i].newsId+'.html'
                        }

                        // list[i].time=new Date(list[i].time).format("yyyy-MM-dd hh:mm:ss");
                        str += '<li> <a class="jie-title" href="'+href +'"'+
                            ' target="_blank">' + list[i].content + '</a>'+
                            '<em><i>'+list[i].time + '</i>'+
                            '<span id="statusAddClass" class="'+colorClass+'">'+list[i].commentstate+'</span>' +
                            '<a class="red-edit" href="javascript:void(0)" onclick="deleteComment('+list[i].id+')">删除</a></em></li>';
                    }
    ...
    function deleteComment(id) {
        parent.layer.confirm('确定删除吗 ?', {
            ...
            $.ajax({
                type : 'post',
                url : '${ctx }deleteMyNewsComment?id=' + id,
                dataType : 'json',
                success : function(result) {
                    if (result.status == 1) {
